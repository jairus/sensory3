...
class Exam extends XY_Controller {
    ...
    public function index($date = NULL) {
        ...
        echo 'hi';
        */
        if(! ($station_number = $this->exam_model->getIfIPHooked())) {
        ...
        $stations = $this->exam_model->getStationsWithQ();
        $this->configXY->JS_VARS['[var=false]EXAM.queue'] = json_encode($stations);
        
        $station_rtas = array(); /* RTAs for this current station number. */
        foreach($stations as $tmp => $data) { if(in_array($station_number, $data['station'])) $station_rtas[] = $data['rta']; }
        
        if(empty($station_rtas)) {
            
            $data['content'] = $this->exam_model->doMessage('<b>This station number has no score-sheet yet.</b>', false);
            $this->load->view('exam', $data);
            return;
        }
        ...
        $station_rtas = $this->exam_model->getStationRTAs($station_rtas);
        if(empty($station_rtas)) {
        ...
        //print_r($station_rtas);
        //exit();
        
        
        if($station_rtas['total'] > 0) {
            
            $this->load->model('sensory_model', '', true);
            $one_ss_only = $this->sensory_model->getSpecifics_withOneSS();
            
            for($x=0; $x<$station_rtas['total']; $x++) {

                $rta = $station_rtas['rows'][$x];
                $q = $this->exam_model->getQ($rta->id);

                $code_1 = xy_code_get($q->codes, 'primary');
                ...
                if(! $check) { 
                    ...
                    $html = '';
                    $specific_as_code = $code_1[0];
                    
                    foreach($code_1 as $code) {
                        
                        $percentage = $this->exam_model->getPercentage($rta->id, $code);
                        $html .= '
                            <tr><td width="50">&nbsp;</td>
                                <td><div><a title="' . $rta->id . ' : ' . $code . ' - ' . $rta->name . '" href="' . xy_url('exam/actual/' . $rta->id . '/' . $code) . '">' . $code . '</a></div></td>
                                <td align="center">' . $percentage . '%</td>
                            </tr>';
                    }
                    
                    $data['qlist'] .= '<tr><td colspan="3"><b>' . $rta->name . '</b></td></tr>' . $html;
                    
                    
                } else {
                    
                    $percentage = $this->exam_model->getPercentage($rta->id, $specific_as_code);
                    $data['qlist'] .= '
                        <tr><td colspan="2"><div style="margin-bottom: 5px"><a title="' . $rta->id . ' : ' . $rta->name . '" href="' . xy_url('exam/actual/' . $rta->id . '/' . $specific_as_code) . '"><b>' . $rta->name . '</b></a></div></td>
                            <td align="center">' . $percentage . '%</td>
                        </tr>';
            ...
            if($data['qlist']) $data['qlist'] = '<table cellpadding="2" cellspacing="0">
                <tr><td colspan="2">Name</td><td>Percentage</td></tr>
                ' . $data['qlist'] . '</table>';
        }
        //print_r($station_rtas);
        ...
        /* START : Get the sequence. */
        /*$sql = "SELECT sequence FROM exams WHERE `date`='" . $date_tmp . "'";
        $query = $this->db->query($sql);
        if($query->num_rows()) {
            
            $sequence = $query->row()->sequence;
            $sequence = str_replace(',0', '', $sequence); /* Removes pause_break in the sequence. */
        //}
        
        /* http://dev.mysql.com/doc/refman/5.5/en/string-functions.html#function_field.
         * Order according to sequence.
         **/
        //if($sequence) $sequence = " AND qd.rta_id IN (" . $sequence . ") ORDER BY FIELD(qd.rta_id," . $sequence . ") ";
        /* END : Get the sequence. */
        
        /*$sql = "
            SELECT  qd.rta_id,
                    rta.samples_name
            FROM    q_distributions qd
            LEFT
            JOIN    rta_forms rta
            ON      qd.rta_id=rta.id
            WHERE   qd.`date`='" . $date_tmp . "' " . $sequence;
        $query = $this->db->query($sql);
        if($query->num_rows()) $q = $query->result();
        else $q = array();
