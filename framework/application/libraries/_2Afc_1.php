...
class _2Afc {
    ...
    function generate($controls, $experiments, $panelists) {
        ...
        foreach($this->patterns as $pattern) {
            ...
            for($x=1; $x<=4; $x++) { /* 4 since, there are 4 possible combinations for 2 product samples. */
                
                if($pattern == 'AB') {
                    
                    if($x < 3) {

                        $tmp[] = $code_arr[0] . ' ' . $experiments[$y - 1];

                    } else {

                        $tmp[] = $code_arr[1] . ' ' . $experiments[$y - 1];
                    }                    
                }
                else
                if($pattern == 'BA') {
                    ...
                    if($x < 3) {
                        ...
                        $tmp[] = $experiments[0] . ' ' . $code_arr[$y - 1];

                    } else {

                        $tmp[] = $experiments[1] . ' ' . $code_arr[$y - 1];
                    }
                }
