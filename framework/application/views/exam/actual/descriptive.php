    ...
    EXAM.axl, {
        ...
        <?php echo $type_ctr?> : function() {
                ...
                if(! EXAM.answers[ctr - 1].axl) {
                    ...
                    Popup.dialog({
                        title : 'ERROR',
                        message : '<div>Please select an answer for "Descriptive #<?php echo $ctr?>: <b><?php echo $label?></b>".</div>',
                        buttons: ['Okay', 'Cancel'],
                        width: '420px'
                    });
    ...
    jQuery('select#<?php echo $type_ctr?>_slider').selectToUISlider({
        ...
        sliderOptions: {
            stop: function(e, ui){
                ...
                var ctr = <?php echo $ctr?>;
                EXAM.answers[ctr - 1]['axl'] = ui.value;
...
<div style="margin-top: <?php echo (($ctr > 1) ? 70 : 10)?>px; margin-left: auto; margin-right: auto; width: 95%">
        ...
        for($x=$min; $x<=$max; $x+=$increment) {
            ...
            if(in_array($x_tmp, array_keys($slider_data))) {
                ...
                if($slider_data[$x_tmp]['visibility'] == 'shown') {
                    ...
                    ?><option<?php echo (($answer == $x_tmp) ? ' selected="selected"' : '')?> value="<?php echo $slider_data[$x_tmp]['label']?>"><?php echo $slider_data[$x_tmp]['label']?></option><?php
                    
                } else {
                    
                    ?><option<?php echo (($answer == $x_tmp) ? ' selected="selected"' : '')?> value=""><?php echo $slider_data[$x_tmp]['label']?></option><?php
                }
                
            } else { ?><option<?php echo (($answer == $x_tmp) ? ' selected="selected"' : '')?> value="">---</option><?php }
        }
