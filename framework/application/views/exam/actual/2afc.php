...
//if(! $controls[0]) $controls[0] = 'Control<br />Not Set';

$codes = $code_choice;//array($experiments[0], $controls[0]); //, $controls[1]
...
jQuery.extend(
    ...
    EXAM.axl, {
        ...
        '_<?php echo $type_ctr?>' : function() {
                
                if(! EXAM.answers[<?php echo $ctr - 1?>].axl) {
                    
                    Popup.dialog({
                        title : 'ERROR',
                        message : '<div>Please select an answer for "2-AFC item# <?php echo $ctr?>".</div>',
                        buttons: ['Okay', 'Cancel'],
                        width: '420px'
                    });
...
<div style="margin-left: auto; margin-right: auto; width: 60%">
    ...
    <div style="margin-left: auto; margin-right: auto; margin-top: 50px; width: 60%">
        <table cellpadding="0" cellspacing="0">
            ...
            foreach($codes as $code) {
                ...
                <td style="text-align: center">
                    ...
                    <div title="<?php echo $code?>" id="<?php echo $type_ctr?>_choice_<?php echo $code?>" style="display: inline-block; margin: 10px; padding: 10px; cursor: pointer; <?php echo $border?>">
                        <div style="font-size: 30px; font-weight: bold; color: #<?php echo ($controls[0] == $code) ? '990000' : '000'?>"><?php echo $code?></div>
                    </div>
            ...
            if($nodiff == 'true') {
                ...
                if($answer == 'No Difference') $border = 'background: #FDF7F7; border: 1px solid #EB9999';
                else $border = 'background: #FFF; border: 1px solid #EFEFEF';
                
                ?>
                <td style="text-align: center">

                    <div title="same" id="<?php echo $type_ctr?>_choice_nodiff" style="display: inline-block; margin: 10px; padding: 10px; min-height: 38px; cursor: pointer; <?php echo $border?>">
                        <div style="font-size: 30px; font-weight: bold; white-space: nowrap; text-transform: uppercase">No Difference</div>
                    </div>
    ...
    <?php if($nodiff_i != '') { ?><div style="margin-top: 20px; font-size: 16px"><b><u>Instruction</u>:</b> <span style="color: #555"><?php echo $nodiff_i?></span></div><?php } ?>    
</div>