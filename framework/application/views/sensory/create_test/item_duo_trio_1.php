...
var ITEM = new function() { 
    ...
    this.ref_code = <?php echo (int) $item['ref_code_value']?>;
...
jQuery(function(){
    ...
    var html_1 = '', html_2 = '', x = 0;
    if(Q.codes_1.length) {
        
        for(x=0; x<Q.codes_1.length; x++) { html_1 += '<div><input type="radio" name="'+ ITEM.type +'_ref_code" id="'+ ITEM.type +'_ref_code_'+ Q.codes_1[x] +'" value="'+ Q.codes_1[x] +'"'+ ((ITEM.ref_code == Q.codes_1[x]) ? ' checked="checked"' : '') +' /> <label for="'+ ITEM.type +'_ref_code_'+ Q.codes_1[x] +'"><b style="font: bold 24px Verdana">'+ Q.codes_1[x] +'</label></div>'; }
        jQuery('#'+ ITEM.type +'_primary_code_wrapper').html(html_1);
    }
...
<div style="clear: both; text-align: left">
    ...
    <table cellspacing="0" cellpadding="0" width="100%">
        ...
        <tr><td style="padding-top: 10px"><b>Settings</b>:</td></tr>
        <tr><td style="padding: 0 0 10px 30px">
                <div style="margin: 10px 0 5px 0">
                    <div id="<?php echo $item_type?>_refcode_label" style="display: inline">Set the reference code:</div>
                </div>
                <table cellpadding="0" cellspacing="0">
                    <!--tr>
                        <td width="200"><b>Primary</b></td>
                        <td><b>Secondary</b></td>
                    </tr-->
                    <tr>
                        <td valign="top" id="<?php echo $item_type?>_primary_code_wrapper"></td>
                        <!--td valign="top" id="<?php echo $item_type?>_secondary_code_wrapper"></td-->
                    </tr>
                </table>
                <div style="margin-top: 10px">
                    <table cellpadding="0" cellspacing="0">
                        <tr>
                            <td><div id="<?php echo $item_type?>_refcode_position_label" style="display: inline">Set the reference code's position:</div></td>
                            <td>
                                <input type="radio" name="<?php echo $item_type?>_ref_code_position" id="<?php echo $item_type?>_ref_code_position_left" value="left"<?php echo (($item['ref_code_position'] == 'left') ? ' checked="checked"' : '')?> /> <label for="<?php echo $item_type?>_ref_code_position_left"><b style="font: bold 15px Verdana">Left</b></label>
                                <input type="radio" name="<?php echo $item_type?>_ref_code_position" id="<?php echo $item_type?>_ref_code_position_center" value="center"<?php echo (($item['ref_code_position'] == 'center') ? ' checked="checked"' : '')?> /> <label for="<?php echo $item_type?>_ref_code_position_center"><b style="font: bold 15px Verdana">Center</b></label>
                                <input type="radio" name="<?php echo $item_type?>_ref_code_position" id="<?php echo $item_type?>_ref_code_position_right" value="right"<?php echo (($item['ref_code_position'] == 'right') ? ' checked="checked"' : '')?> /> <label for="<?php echo $item_type?>_ref_code_position_right"><b style="font: bold 15px Verdana">Right</b></label>
                            </td>
                        </tr>
                    </table>
