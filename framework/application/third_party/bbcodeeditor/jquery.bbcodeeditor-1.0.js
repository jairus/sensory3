(function($) {
	...
	{	
		...
		if(options.dsize != false) options.dsize.click(function() {font_size(false)});
          if(options.tcolor != false) options.tcolor.click(function() {print_bbc('text','[color=red]','[/color]');});
	...
	$.fn.bbcodeeditor.preview = function() {
		if(options.preview != false) 
		{
			...
			var replace	= ['<b>$1<\/b>',
			           	   ...
			           	   '<img $1 src="$2" />',
			           	   '<pre><code>$1</code></pre>',
			           	   '<blockquote style="border:1px solid #CCC; padding:4px;">$2</blockquote>',
			           	   '[list$1]$2<li>$3</li>$4',
			           	   '<ul>$1</ul>',
			           	   '<ol start=$1>$2</ol>',
			           	   '%lb%%lb%'
			...
			for(var i in find)
			{
				...
				if(i == 9) while(txt.match(find[i],replace[i])) txt = txt.replace(find[i],replace[i]); // fix [*] so that they only work inside [/list]
			}
